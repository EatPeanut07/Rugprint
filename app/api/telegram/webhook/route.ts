import{NextResponse}from"next/server";import{linkTelegram,subscriptionByTelegram,deliverAlert}from"@/lib/alert-store";import{sendTelegramMessage,rugPrintKeyboard,setTelegramCommands}from"@/lib/telegram";
...
else if(cmd==="/help"||t==="❓ Help"){await menu(c,"❓ <b>RugPrint Help</b>\n\n🔎 Check Creator opens Creator Intelligence.\n🚨 My Alerts opens your alert dashboard.\n📊 Status shows your plan and monthly usage.\n📝 Report Creator opens evidence reporting.\n💎 Upgrade to Pro shows the current Pro offer.\n\nRugPrint separates community reports from verified findings. Evidence first.");}
else if(t==="🔎 Check Creator"){await sendTelegramMessage(c,`🔎 <b>Creator Intelligence</b>\n${base()}/creator`,{reply_markup:rugPrintKeyboard});}
else if(t==="🚨 My Alerts"){await sendTelegramMessage(c,`🚨 <b>RugPrint Alerts</b>\n${base()}/alerts`,{reply_markup:rugPrintKeyboard});}
else if(t==="📝 Report Creator"){await sendTelegramMessage(c,`📝 <b>Submit evidence</b>\n${base()}/report\n\nCommunity reports remain unverified unless RugPrint verifies the evidence.`,{reply_markup:rugPrintKeyboard});}
else if(t==="💎 Upgrade to Pro"){await menu(c,"💎 <b>RugPrint Pro</b>\nOne-off price: <b>0.5 SOL</b>\nUnlimited delivered Telegram alerts.\n\nAutomatic in-bot payment activation is being connected. Do not send funds until RugPrint provides a payment reference.");}
